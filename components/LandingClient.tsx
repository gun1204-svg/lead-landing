...
const concernOptions04 = [
  {
    title: "첫 수술 후 코 모양이 마음에 들지 않아요",
    desc: "수술 후 전체적인 코 라인이나 분위기가 만족스럽지 않은 경우",
  },
  {
    title: "코끝이 처지거나 들려 보여요",
    desc: "시간이 지나며 코끝 방향이나 높이가 아쉬워진 경우",
  },
  {
    title: "보형물이 비치거나 휘어 보여요",
    desc: "보형물 경계가 보이거나 중심이 틀어져 보이는 경우",
  },
  {
    title: "콧대·코끝 라인이 부자연스러워요",
    desc: "전체적인 연결감이나 라인이 어색하게 느껴지는 경우",
  },
  {
    title: "재수술이 필요한지 먼저 상담받고 싶어요",
    desc: "현재 상태에서 재수술 가능 여부가 궁금한 경우",
  },
];
...
const concernOptions08 = [
  {
    title: "코 모양은 바꾸고 싶지만 절개 흉터가 걱정돼요",
    desc: "겉으로 보이는 절개 부위에 대한 부담이 있어 비개방 수술 가능 여부가 궁금한 경우",
  },
  {
    title: "자연스럽게 콧대와 코끝 라인을 개선하고 싶어요",
    desc: "과한 변화보다는 현재 얼굴과 조화를 이루는 자연스러운 코 라인을 원하는 경우",
  },
  {
    title: "비개방으로 수술이 가능한 코인지 궁금해요",
    desc: "현재 코 상태에서 비개방 방식으로 원하는 개선이 가능한지 먼저 확인하고 싶은 경우",
  },
  {
    title: "코수술 후 회복 과정이 걱정돼요",
    desc: "수술 방법에 따른 회복 과정과 주의사항을 충분히 상담받은 뒤 결정하고 싶은 경우",
  },
  {
    title: "콧대·코끝 등 필요한 부분만 개선하고 싶어요",
    desc: "전체적인 변화보다 신경 쓰이는 부분을 중심으로 개선 방향을 상담받고 싶은 경우",
  },
  {
    title: "제 코에 맞는 수술 방법부터 상담받고 싶어요",
    desc: "비개방과 개방 방식 중 어떤 방법이 현재 코 상태에 적합한지 먼저 상담받고 싶은 경우",
  },
];
...
function LandingSpecialContent({
  ...
  landingKey: "02" | "03" | "04" | "05" | "06" | "07" | "08" | "10";
  concerns: string[];
  ...
  const isLanding08 = landingKey === "08";
    ...
    : isLanding04 || isLanding05
      ...
      : isLanding06
        ...
        : isLanding07
          ...
          : isLanding08
            ? "비개방 코성형에서 가장 궁금하거나 개선하고 싶은 부분을 선택해 주세요."
            : isLanding10
              ? "현재 가장 신경 쓰이는 고민을 선택해 주세요."
    ...
    : isLanding04 || isLanding05
      ...
      : isLanding06
        ...
        : isLanding07
          ...
          : isLanding08
            ? "선택하신 비개방 코성형 고민 기준으로 상담 안내가 진행됩니다."
            : isLanding10
  ...
  return (
    <>
        ...
        <section>
          <img
            ...
            onError={(e) => {
              ...
              e.currentTarget.src = "/intro/08/01.png";
            }}
            alt="비개방 코성형 상담 01"
      ...
      {isLanding08 && (
        <>
          {["02", "03", "04", "05", "06", "07"].map((imageNo) => (
            <section key={imageNo}>
              <img
                src={`/intro/08/${imageNo}.jpg`}
                onError={(e) => {
                  e.currentTarget.onerror = null;
                  e.currentTarget.src = `/intro/08/${imageNo}.png`;
                }}
                alt={`비개방 코성형 상담 ${imageNo}`}
                className="block w-full"
                draggable={false}
              />
            </section>
      ...
      {!isLanding10 && !isLanding05 && !isLanding06 && !isLanding07 && !isLanding08 && (
      ...
      {!isLanding05 && !isLanding06 && !isLanding07 && !isLanding08 && (
...
export default function LandingClient({ landingKey }: { landingKey: string }) {
  ...
  const isLanding08 = config.key === "08";
  ...
  const showKakaoAfterSuccess =
    isLanding02 || isLanding03 || isLanding05 || isLanding06 || isLanding07 || isLanding08;
  ...
  const [concerns08, setConcerns08] = useState<string[]>([]);
  ...
  function toggleConcern08(item: string) {
    setConcerns08((prev) =>
      prev.includes(item) ? prev.filter((v) => v !== item) : [...prev, item],
    );
  }
